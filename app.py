...
def load_member_info(member, order):
    member_info = {"trophies": member["trophies"] if "trophies" in member else 0,
                   "town_hall": member["town_hall"], "name": member["name"],
                   "percentage_missed": f"{round((member['misses'] / member['total']) * 100)}%" if "misses" in member else "",
...
def index():
    ...
    for member in clash:
        if clash[member]["in_clan"] and "trophies" in clash[member]:
